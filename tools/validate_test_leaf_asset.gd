...
func _init() -> void:
	var asset := load("res://assets/vegetation/sm_test_leaf_test2_asset.tres") as AutoVegetationAsset
	if asset == null:
		push_error("Failed to load sm_test_leaf_test2 AutoVegetationAsset")
	...
	print("asset_id=%s subtype=%s band=%s" % [asset.asset_id, asset.object_subtype, asset.vegetation_band])
