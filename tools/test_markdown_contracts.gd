...
func _test_routing_probe_sources_are_sv_resident_and_descriptor_probe_backed() -> bool:
	...
	var candidate_docs := {
		...
		"asset semantic probes": TestUtils.read_text("res://demos/asset-descriptor-demo/asset-semantic-probes.md"),
		"autoobject probe prefilter": TestUtils.read_text("res://demos/placement-autoobject-probe-prefilter/autoobject-probe-prefilter.md"),
		"target projection": TestUtils.read_text("res://demos/target-sv-point-cloud-conversion-c/target-scene-voxel-projection.md"),
		"meshfill framework": TestUtils.read_text("res://demos/core-meshfill-framework/meshfill-framework.md"),
	}
	var candidate_graphs := {
		"runtime architecture graph": TestUtils.read_text("res://demos/core-SPA-scene-placement-actor/diagrams/autoobject_gpu_runtime_architecture.svg"),
		"prefilter graph": TestUtils.read_text("res://demos/placement-autoobject-probe-prefilter/diagrams/autoobject_probe_prefilter_pipeline.svg"),
		"framework graph": TestUtils.read_text("res://demos/core-meshfill-framework/diagrams/meshfill_current_framework.svg"),
		"routing graph": TestUtils.read_text("res://demos/placement-voxel-semantic-routing/diagrams/voxel-semantic-routing.svg"),
	}

	for missing in DemoContractUtils.find_missing_terms(prefilter_source, ["sv.get(\"complexity_field\"", "sv.get(\"collision_field\"", "get_semantic_probes"]):
		push_error("  FAIL: probe prefilter source is missing '%s'" % missing)
		ok = false

	for missing in DemoContractUtils.find_missing_terms(score_probe_shader, ["buffer ProbeData", "buffer ComplexityCollision", "buffer TargetField"]):
	...
	for missing in DemoContractUtils.find_missing_terms(target_projection_doc, [
	...
	var target_point_cloud_contract := "\n".join([
		TestUtils.read_text("res://demos/target-sv-point-cloud-conversion-c/target-sv-point-cloud-conversion.md"),
		TestUtils.read_text("res://demos/target-sv-point-cloud-conversion-c/target-sv-point-cloud-conversion.tscn"),
		TestUtils.read_text("res://tools/test_target_sv_point_cloud_conversion.gd"),
	])
	for missing in DemoContractUtils.find_missing_terms(target_point_cloud_contract, [
...
func _test_gpu_first_no_cpu_fallback_contracts() -> bool:
	...
	var ok := true
	var docs_combined := "\n".join([
		TestUtils.read_text("res://demos/asset-descriptor-demo/asset-properties.md"),
		TestUtils.read_text("res://demos/asset-descriptor-demo/asset-semantic-probes.md"),
		TestUtils.read_text("res://demos/core-meshfill-framework/meshfill-framework.md"),
		TestUtils.read_text("res://demos/core-SPA-scene-placement-actor/autoobject-gpu-runtime-architecture.md"),
		TestUtils.read_text("res://demos/core-scenevoxeltile/scenevoxeltile.md"),
		TestUtils.read_text("res://demos/core-SPA-scene-placement-actor/core-scene-placement-actor.md"),
	])
	for missing in DemoContractUtils.find_missing_terms(docs_combined, [
		"GPU-first",
		"CPU 只负责",
		"staging",
		"debug readback",
	...
	for present in DemoContractUtils.find_present_terms(docs_combined, [
		"cpu-only",
		"cpu only",
		"cpu fallback",
		"gpu_resident=false",
	...
	for test_spec in [
		...
		var test_text := TestUtils.read_text(path)
		...
		if test_text.find(skip_text) < 0:
			...
			ok = false
		if TestUtils.mentions_no_rd_cpu_success(test_text.to_lower()):
	...
	for demo_path in [
		...
		var demo_text := TestUtils.read_text(demo_path)
		for missing in DemoContractUtils.find_missing_terms(demo_text, ["--rendering-driver vulkan", "RenderingDevice"]):
...
func _test_legacy_scene_voxel_scatter_removed() -> bool:
	...
	var ok := true
	var committer_source := TestUtils.read_text("res://scripts/scene_voxel_committer.gd")
	for present in DemoContractUtils.find_present_terms(committer_source, [
...
func _test_vpg_and_scene_tile_gpu_binding_contracts() -> bool:
	...
	var runtime_source := TestUtils.read_text("res://scripts/gpu_autoobject_runtime.gd")
	for missing in DemoContractUtils.find_missing_terms(runtime_source, [
	...
	var profile_container_source := TestUtils.read_text("res://scripts/auto_voxel_runtime_profile_container.gd")
	for missing in DemoContractUtils.find_missing_terms(profile_container_source, [
	...
	var active_spatial_sources := "\n".join([
		...
		runtime_source,
		committer_source,
		TestUtils.read_text("res://scripts/autoobject_probe_prefilter_gpu.gd"),
...
func _test_demo_doc_test_commands_match_driver_contract() -> bool:
	...
	var seen := {}
	for doc_path in TestUtils.collect_files("res://demos", ".md", [], "  FAIL: cannot open demo doc root"):
		var text := TestUtils.read_text(doc_path)
