...
func _init() -> void:
	...
	var footprint: Array[Dictionary] = [
		{"local_pos": Vector3i(0, 0, 0), "collision_degree": 64, "flags": FLAG_SUPPORT, "weight": 1.0},
		{"local_pos": Vector3i(0, 1, 0), "collision_degree": 255, "flags": 0, "weight": 1.0},
		{"local_pos": Vector3i(0, 2, 0), "collision_degree": 0, "flags": FLAG_CLEARANCE, "weight": 1.0},
	]
	...
	for ch in range(VoxelPlacementGeneratorScript.NUM_DEBUG_CHANNELS):
		...
		print("[TargetDebugTest] ch %d (%s): max=%.4f" % [ch, names[ch], max_val])
	...
	no_target_settings.erase("target_occupancy")
	no_target_settings.erase("target_color")
