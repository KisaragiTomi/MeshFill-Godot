...
func _run() -> int:
	...
	if textures.is_empty():
		push_error("[CliffGenTest] Failed to load terrain textures")
		main.free()
		return FAILED

	var cliff_meshes: Array[Mesh] = []
	var assets: Array[AutoRock] = []
	main._load_cliff_data(cliff_meshes, assets)
	if assets.is_empty():
		push_error("[CliffGenTest] No cliff assets loaded")
		main.free()
		return FAILED
	...
	generator.rock_overlap = main.rock_overlap
	generator.rock_assets = assets

	var results := generator.generate_cliff_vertical(3)
	if generator._debug_target_height_image == null or generator._debug_current_height_image == null:
		push_error("[CliffGenTest] Missing debug readback images")
		generator.free()
		main.free()
		return FAILED
	...
	for result in results:
		...
		if mesh_index < 0 or mesh_index >= assets.size():
			push_error("[CliffGenTest] Invalid mesh_index in result: %d" % mesh_index)
			generator.free()
			main.free()
			return FAILED
		var scale: Vector3 = result.get("scale", Vector3.ZERO)
		if scale.x <= 0.0 or scale.y <= 0.0 or scale.z <= 0.0:
			push_error("[CliffGenTest] Non-positive result scale: %s" % [scale])
			generator.free()
			main.free()
			return FAILED
		...
		if scale.x < expected_min or scale.x > expected_max:
			push_error("[CliffGenTest] Result scale %.3f outside asset random_scale %s" % [scale.x, asset.random_scale])
			generator.free()
			main.free()
			return FAILED
	...
	if results.size() > 1 and max_seen_scale - min_seen_scale < 0.02:
		push_error("[CliffGenTest] Result scales did not vary enough: %.3f..%.3f" % [min_seen_scale, max_seen_scale])
		generator.free()
		main.free()
		return FAILED

	print("[CliffGenTest] assets=%d placements=%d scale=%.3f..%.3f target=%dx%d current=%dx%d" % [
		assets.size(),
		results.size(),
	...
	generator.rock_assets.clear()
	generator._active_rock_assets.clear()
	generator.free()
	for asset in assets:
