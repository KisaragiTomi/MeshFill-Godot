...
func _run() -> void:
	...
	var ok := paint_ok
	var paint_diff := ShotUtils.image_diff_ratio(before.get("image", null), rgb.get("image", null))
	...
	var complexity_diff := ShotUtils.image_diff_ratio(rgb.get("image", null), complexity.get("image", null))
	...
	var collision_diff := ShotUtils.image_diff_ratio(complexity.get("image", null), collision.get("image", null))
	...
	var rgb_return_diff := ShotUtils.image_diff_ratio(rgb.get("image", null), rgb_again.get("image", null))
...
func _shot(tag: String) -> Dictionary:
	await _settle(3)
	var result := ShotUtils.save_viewport_png(_vp, "%s/%s.png" % [SHOT_DIR, tag], false, true)
