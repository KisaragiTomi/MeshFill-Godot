...
func _init() -> void:
	...
	compact_settings["candidate_tiles"] = [Vector3i(0, 0, 0)]
	...
	if compact.is_empty() or int(compact.get("candidate_tile_count", 0)) != 1:
		push_error("[VoxelPlacementTest] Expected compact candidate tile dispatch")
		quit(1)
		return
	...
	print("[VoxelPlacementTest] first=%s second=%s compact=%s stamped=%d deltas=%d tiles=%d compact_tiles=%d" % [
		...
		int(compact.get("candidate_tile_count", 0)),
	])
	quit(0)
