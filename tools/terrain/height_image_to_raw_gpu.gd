...
static func convert_height_image_file_gpu(
	...
	var img := Image.new()
	var err := img.load(src_path if _is_virtual_path(src_path) else resolved_src)
	if err != OK:
		return _blocked("source_image_load_failed")
	...
	if out_file == null:
		return _blocked("destination_open_failed", int(result.get("width", 0)), int(result.get("height", 0)))
