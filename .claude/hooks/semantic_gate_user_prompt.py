...
merge) before coining new vocabulary.

Differences from the codex version:
  * Calls the self-contained interpreter ``D:\\.aidata\\Python311`` explicitly
    (``python -m mempalace``) instead of relying on a ``mempalace`` console
    script being on PATH.
  * Pins ``MEMPALACE_PALACE_PATH`` to the shared palace in the child env, so
    the search resolves to ``D:\\.aidata\\.mempalace\\palace`` regardless of
    the parent process environment.

Reads the hook JSON payload on stdin, emits a Claude Code
``hookSpecificOutput.additionalContext`` object on stdout (empty ``{}`` when
there is nothing to add). Fails open: any error path emits ``{}``.
"""
import json
import os
import re
...
WING = "MeshFill-Godot"
PYEXE = r"D:\.aidata\Python311\python.exe"
PALACE = r"D:\.aidata\.mempalace\palace"
...
def build_queries(prompt: str) -> list:
    queries = []
