...
```powershell
python "D:\.aidata\skills\skill-rag\skill_rag.py" query "你的查询" --top-k 3
```

JSON 输出（适合程序解析）：

```powershell
python "D:\.aidata\skills\skill-rag\skill_rag.py" query "你的查询" --top-k 3 --json
```

### 重建索引（新增/修改 Skill 后）

```powershell
python "D:\.aidata\skills\skill-rag\skill_rag.py" index
```
