...
## 七、Buffer 合并记录 (Consolidation Log)

### 7.1 scene_buf + collision_buf → scene_collision_buf (2026-06-07)

**合并前**: 两个独立的 `float` buffer（各 `voxel_count * 4` bytes）

**合并后**: 一个 interleaved `vec2` buffer（`voxel_count * 8` bytes）

**Layout**: `(scene_occ[0], collision_occ[0], scene_occ[1], collision_occ[1], ...)`

**影响的 shader**:
| Shader | 原 binding | 新 binding | 访问方式 |
|--------|----------|----------|----------|
| `collect_sv_anchors.glsl` | 0=SceneOcc, 1=CollisionOcc | 0=SceneCollision | `scene_coll[i].x` / `.y` |
| `score_anchor_asset_probes.glsl` | 3=SceneOcc, 4=CollisionOcc | 3=SceneCollision | `scene_coll[i].x` / `.y` |

**Binding 重编号** (数据依赖移位):
- `collect_sv_anchors.glsl`: TargetField 2→1, DirtyTiles 3→2, AnchorOut 4→3, AnchorCount 5→4
- `score_anchor_asset_probes.glsl`: TargetField 5→4, ScoresOut 6→5

**收益**: 减少 2 个 GPU buffer 分配（1 次上传替代 2 次），减少 1 个 shader binding

### 7.2 count_buf → debug_buf 合并 (2026-06-07)

**合并前**: `count_buf` (16 bytes) + `debug_buf` (64 bytes) = 两个独立 buffer

**合并后**: `debug_buf` (64 bytes)，前 16 bytes 为 count fields

**Layout**:

| u32 index | byte offset | 字段 |
|-----------|-------------|------|
| 0 | 0 | record_count |
| 1 | 4 | positive_vote_count |
| 2 | 8 | duplicate_tile_id_count |
| 3 | 12 | overflow_count |
| 4 | 16 | magic |
| 5 | 20 | asset_count |
| 6 | 24 | tile_count |
| 7 | 28 | record_capacity |
| 8 | 32 | written_records |
| 9 | 36 | skipped_empty_tiles (expand only) |

**影响的 shader**:
| Shader | 原 binding | 新 binding | 变化 |
|--------|----------|----------|------|
| `pack_candidate_route_records_from_votes.glsl` | 5=counts, 6=debug | 5=debug (merged) | counts 写入 debug[0..3] |
| `expand_scene_voxel_tile_routes.glsl` | 6=counts, 7=debug | 6=debug (merged) | counts 写入 debug[0..3] |

**收益**: 减少 1 个 GPU buffer 分配，减少 1 个 shader binding

### 7.3 汇总

| 指标 | 合并前 | 合并后 | 变化 |
|------|--------|--------|------|
| FRAME buffer 数量（Prefilter 2.3） | 17 | 15 | -2 |
| collect_sv_anchors bindings | 6 (0-5) | 5 (0-4) | -1 |
| score_anchor_asset_probes bindings | 7 (0-6) | 6 (0-5) | -1 |
| pack shader bindings | 7 (0-6) | 6 (0-5) | -1 |
| expand shader bindings | 8 (0-7) | 7 (0-6) | -1 |
| GPU buffer 分配（每次 prefilter） | 减少 3 次 allocation | — | — |
