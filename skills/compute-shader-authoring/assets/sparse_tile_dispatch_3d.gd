class_name SparseTileDispatch3D
...
func _init(
	...
	log_name = "SparseTileDispatch3D"
	_rd = rendering_device
	_owns_rendering_device = false
	configure(gw, gh, gd, p_tile_size, p_expand, p_threshold, p_mask_threshold, p_activity_mode)
...
func configure(
	...
	groups_z = ceil_div(grid_d, tile_size)
	max_tiles = groups_x * groups_y * groups_z
...
func dispatch_compact(cl: int) -> void:
	...
	push.encode_s32(8, grid_d)
	push.encode_s32(12, tile_size)
	push.encode_s32(16, expand_voxels)
	push.encode_s32(20, activity_mode)
	push.encode_s32(24, 1 if use_mask else 0)
	push.encode_s32(28, 0)
	push.encode_float(32, threshold)
	push.encode_float(36, mask_threshold)
	...
	_rd.compute_list_set_push_constant(cl, push, 48)
...
func readback_tile_coords(count: int) -> Array[Vector3i]:
	if _rd == null or count <= 0:
		return []
	var clamped := mini(count, max_tiles)
	var bytes := _rd.buffer_get_data(buf_compact_tile_coords, 0, clamped * 16)
	var result: Array[Vector3i] = []
	result.resize(clamped)
	for i in range(clamped):
		var base := i * 16
		result[i] = Vector3i(
			bytes.decode_u32(base),
			bytes.decode_u32(base + 4),
			bytes.decode_u32(base + 8)
		)
	return result
...
func _create_buffers() -> void:
	...
	buf_compact_tile_coords = storage_buffer_zero(max_tiles * 16, SCOPE_PERSISTENT, "compact_tile_coords")
...
func _load_shaders() -> void:
	_shader_compact = load_compute_shader("res://shaders/std_compact_generic_3d.glsl")
	if _shader_compact.is_valid():
		_pipeline_compact = create_compute_pipeline(_shader_compact, SCOPE_PERSISTENT, "compact_3d")
