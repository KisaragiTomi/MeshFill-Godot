...
`SemanticProbeProfile.generate_from_mesh()` 按优先级生成候选点。collision 输入来自 [`AssetDescriptor.collision`](auto-voxel-descriptor.md)；`collision` 只作为 placement footprint API 命名保留。
