...
这份文档把新增岩石 / 新增植被的手工步骤收拢到脚本入口，并同步当前字段契约。所有资产种类的默认语义定义见 [`auto-voxel-descriptor.md`](auto-voxel-descriptor.md)。
...
- [`AutoVoxelDescriptor`](auto-voxel-descriptor.md) 是资产默认体素语义的 canonical source；脚本化资产不要把 profile 或 metadata 当成第二套权威。
...
- `AutoRock` 保留 rock 专用 façade；descriptor-backed 资产直接实例化为 `AutoObject`，共享语义交给 descriptor-backed 字段。
...
`AutoAssetFactory.create_or_update_object_asset()` 先从 profile 取 `color` / `complexity` / `collision` fallback，再用显式 `collision` 覆盖 collision，并把结果归一化后交给 `configure_asset()` / `configure_object()`。`sync_exported_fields` 只同步 Inspector mirror；不要把 `voxel_profile` 写成唯一语义权威。
...
| Output | Meaning |
| --- | --- |
| `script_path` | 可选 `AutoObject` façade 子类，例如 `AutoFlower`。 |
| `asset_path` | `AutoVoxelDescriptor` 资源；字段分组见 [`auto-voxel-descriptor.md`](auto-voxel-descriptor.md)。 |
...
add_child(flower)
register_brush_autoobject(flower, flower_asset.make_instance_config())
```
...
`AutoRock` 会在 record extra fields 中补 `mesh_index`；descriptor-backed `AutoObject` 可以通过 instance config 提供 `object_subtype`、`channel` 和 `radius`。这些是 runtime record extra，不进入 `SharedPropertyType.SHARED_FIELD_KEYS`。

## Metadata Rule

脚手架和运行时代码不把 metadata 当作第二套配置面。新增资产语义写入 [`AutoVoxelDescriptor`](auto-voxel-descriptor.md) 或 typed asset 的 descriptor-backed 字段；实例落地后的颜色、collision、像素和 slice 写入 `instance_stamp_write_spec` / `ISWS`。metadata 只保留 id、来源、`object_type`、实例 id 和 `ISWS` 查询入口；`instance_stamp_write_spec` 是 preferred metadata key，旧 `voxel_write_spec` metadata key 只作为兼容别名。
