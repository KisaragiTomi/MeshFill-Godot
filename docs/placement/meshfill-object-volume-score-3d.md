...
> 状态：**设计 / 计划阶段，未实现**。当前 in-tree 的体素评分仍是 `shaders/score_voxel_tile.glsl` 的 `8^3` tile 粒度（`TILE_SIZE = 8`，见 [`voxel-semantic-routing.md`](voxel-semantic-routing.md)）。本文描述的 `50^3` 单物体体积评分是接替 2.5D heightfield fitting 的新 score 路径，尚未落地为 shader / GDScript。
...
| 常量 | 值 | 说明 |
| --- | --- | --- |
| `OBJECT_SAMPLE_EXTENT` | `50` | sampling volume 每轴 voxel 数。 |
| `SUBTILE_SIZE` | `8` | subtile 每轴 voxel 数；对齐 `local_size`。 |
| `SUBTILES_PER_AXIS` | `7` | `ceil(50 / 8)`；末块越界 voxel 由 guard 跳过。 |
| `SUBTILE_COUNT` | `343` | `7^3`，每个候选物体的 subtile 数。 |
| `ROTATION_SLOTS` | `12` | 一次评测的旋转角度数。 |
| `LOCAL_SIZE` | `8 x 8 x 8 = 512` | 每个 subtile workgroup 的线程数。 |

`OBJECT_SAMPLE_EXTENT` / `ROTATION_SLOTS` / yaw 轴向都应作为 push constant 或 settings 可调，不要硬编码进 shader 逻辑分支。
...
- `50^3` 是否对所有 asset 固定，还是按 asset footprint AABB + context radius 动态裁剪 sampling volume 上限。
...
- forward / inverse footprint 旋转哪种在 `8^3` shared cache 下访存更优，需 RenderDoc / readback 实测。
