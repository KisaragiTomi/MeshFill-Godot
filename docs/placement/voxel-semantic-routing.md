# Voxel Semantic Routing — 候选资产路由与 Voxel 区域路由
...
本文不再假设通过语义匹配从全资产库选择“最优 asset”。前置筛选阶段已经根据 anchor、probe、TargetSV、规则或资产 profile 得到了可用候选资产；后续语义向量匹配只在这些候选资产内部做 rerank、validate 或 prune。本文负责把保留下来的候选资产整理成可被 placement 使用的候选 voxel 区域。
...
- 候选 voxel 区域必须偏向召回，按 footprint、probe 插值采样范围和 context 半径保守扩张，不能只保留 anchor 所在区域。
- `EMPTY_ASSET_ID` 是合法结果，用于表示某些 anchor / voxel 区域没有有效候选或应保持为空。
...
候选 voxel 区域不是精确采样点集合。Asset probes 后续会对 `TargetSV` / `SceneVoxel` 做插值采样，因此路由输出必须保守扩张：
...
- `context_sensing_radius > 0` 的小型资产要把外围 context probe 感受野也纳入候选区域。
- 候选区域应宁可多包含 voxel，交给 `score_voxel_tile.glsl` 精筛；不要在 routing 阶段过早裁剪导致高分位置无法被采样。

### 同类型 AutoObject 互斥预检

当前实现中，`VoxelPlacementGenerator.run_multi_asset()` 在进入 GPU physical scoring 之前，可以先用 `AutoObjectManager` 对候选 voxel 区域做同类型互斥预检。该步骤不替代 `score_voxel_tile.glsl` 的 footprint / support / collision / clearance 精筛，只负责剪掉已经明显违反同类中心距约束的候选区域。
...
如果候选 voxel 区域被同类型对象阻挡，该区域会在 physical placement 前被移除；如果该 asset 的所有候选区域都被移除，本轮该 asset 返回：
...
语义匹配阶段不直接输出唯一 `best_asset`。它应保留多个候选 route，让后续 voxel 区域聚合和 `score_voxel_tile.glsl` 继续处理 footprint、support、collision、clearance 等物理精筛。
...
> 待检查：这一层 target context pooling 目前更像候选内二次验证，与 probe 对 TargetSV 的采样语义边界需要重新澄清。当前按计划项理解，不作为已定稿主路径。
...
Voxel 区域聚合时，`EMPTY` 不作为 asset 输出；但如果一个候选区域内 EMPTY 投票过高，则整个区域可跳过候选放置。
