...
func _init_gpu() -> void:
	...
	if _gpu_ready:
		...
		print("[SceneVoxelCommitter] GPU compute ready (10 shaders; tile→SceneVoxelTileStore, source→SceneVoxelSourceStaging)")
...
func _collision_layer_base_px(base_px: Vector2i, collision: Dictionary) -> Vector2i:
	...
	return VoxelGeneralScript.collision_layer_base_px(base_px, collision, _base_res)

## 计算碰撞记录的像素半径，点采样优先用 radius_px 字段
func _collision_radius_px(collision: Dictionary) -> int:

	return VoxelGeneralScript.collision_radius_px(collision, _capture_size, _base_res)
