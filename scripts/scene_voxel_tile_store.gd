...
## 通过 _committer 反向引用读取 committer 的 grid 等上下文。
...
## 瓦片 compute shader 就绪标志(本组件自有的 4 个 tile shader)
...
## 由 committer 调用：绑定反向引用并加载 tile compute shader。
## 调用前 committer 须已 attach_rendering_device(_rd, false)。
func setup(committer) -> void:
	_committer = committer
	log_name = "SceneVoxelTileStore"
	_init_tile_gpu()

## 加载本组件负责的 4 个 tile compute shader 与管线，置 _gpu_ready。
...
func _scene_voxel_tile_bounds_from_record(record: Dictionary) -> Dictionary:
	if SceneVoxelTileCodecScript.has_any_key(record, ["voxel_min", "bounds_min", "new_voxel_min", "new_bounds_min"]) and SceneVoxelTileCodecScript.has_any_key(record, ["voxel_max", "bounds_max", "new_voxel_max", "new_bounds_max"]):
		var explicit_min := SceneVoxelTileCodecScript.first_vector3i(record, ["voxel_min", "bounds_min", "new_voxel_min", "new_bounds_min"], Vector3i.ZERO)
		var explicit_max := SceneVoxelTileCodecScript.first_vector3i(record, ["voxel_max", "bounds_max", "new_voxel_max", "new_bounds_max"], explicit_min + Vector3i.ONE)
	...
	if base_px is Vector2i:
		...
		if voxel_xz is Vector2i:
			...
			if source_res != xz_res:
				center_px = VoxelGeneral.base_pixel_to_volume_pixel(voxel_xz, source_res, xz_res)
			else:
...
func _append_scene_voxel_tile_record_refs(tile: Dictionary, record: Dictionary, unique_source: bool = false) -> Dictionary:
	var debug_id := SceneVoxelDebugScript.tile_object_id(record)
	var ids: Array = tile.get("auto_object_ids_debug", [])
	SceneVoxelDebugScript.append_tile_value(ids, debug_id, true)
...
func _append_scene_voxel_tile_record_refs_for_bounds(
	...
	if SceneVoxelDebugScript.tile_object_id(record).is_empty() and SceneVoxelDebugScript.tile_source_id(record).is_empty():
...
func _rebuild_scene_voxel_tile_compact_ranges() -> void:
	...
	for tile_key in tile_keys:
		...
		var object_debug_range := SceneVoxelDebugScript.append_tile_range(
...
## 统一清除全部 SceneVoxelTile 脏标志；由 committer 提交路径与 clear_sv_dirty 调用
...
func ensure_scene_voxel_tile_buffers_uploaded(force: bool = false) -> bool:
	...
	var tile_ids := SceneVoxelTileCodecScript.sorted_tile_ids(_scene_voxel_tiles)
	_scene_voxel_tile_gpu_tile_ids = tile_ids.duplicate()
	var packed_records := SceneVoxelTileCodecScript.pack_record_bytes(tile_ids, _scene_voxel_tiles, _scene_voxel_tile_size())
	var packed_summaries := SceneVoxelTileCodecScript.pack_summary_bytes(tile_ids, _scene_voxel_tiles)
...
## 常驻 field buffer 的目标体素数（= committer grid_size 的体素总数）
func _resident_field_voxel_count() -> int:
	return VoxelGeneral.voxel_count(_committer.grid_size)
...
## 碰撞常驻场的初始内容：committer 级地形基底图（_terrain_base_collision_field，真身在
## field_builder）按 grid_size 分辨率经 3D 种子 pass 打包为每体素 u32 字节；
## 基底图为 null/空时返回全零种子（保持既有语义）。
## 种子 shader(terrain_collision_volume.glsl) 按 texelFetch 直取源纹理，不做缩放，
## 因此基底图分辨率 != grid xz 分辨率时先重采样到 grid 分辨率再喂入。
func _seed_collision_field_base_bytes(voxel_count: int) -> PackedByteArray:
	var seed_bytes := PackedByteArray()
	seed_bytes.resize(SceneVoxelTileCodecScript.u32_field_byte_count(voxel_count))
	var terrain_base_img: Image = _committer._terrain_base_collision_field
	if terrain_base_img == null or terrain_base_img.is_empty():
		return seed_bytes
	var xz_res := maxi(_committer.grid_size.x, 1)
	var total_slices := maxi(_committer.grid_size.y, 1)
	if terrain_base_img.get_width() != xz_res or terrain_base_img.get_height() != xz_res:
		terrain_base_img = _committer._field_builder._resample_collision_field(terrain_base_img, xz_res)
		if terrain_base_img == null or terrain_base_img.is_empty():
			return seed_bytes
	...
	if base_field.size() != voxel_count:
		return seed_bytes
	return SceneVoxelTileCodecScript.pack_collision_field_u32_bytes(base_field, voxel_count)
...
## 设置 tile GPU 缓冲自动上传开关；enabled 且 upload_now 时立即上传一次
...
func _create_scene_voxel_tile_storage_buffer(buffer_name: String, bytes: PackedByteArray, record_count: int, stride_bytes: int) -> bool:
	...
	_scene_voxel_tile_gpu_buffer_hashes[buffer_name] = HashUtils.stable_u32_from_bytes(bytes)
	return true
...
## 将回读字节解码为浮点字段（scene_voxel_debug 快照回读用）
...
func _decode_scene_voxel_tile_collision_field_bytes(bytes: PackedByteArray, voxel_count: int) -> PackedFloat32Array:
	return SceneVoxelTileCodecScript.decode_collision_field_u32_bytes(bytes, voxel_count)
...
func _decode_scene_voxel_tile_compact_summaries(bytes: PackedByteArray, record_count: int, tile_grid: Vector3i) -> Array[Dictionary]:
	...
	for record_index in range(available):
		...
		summaries.append({
			# reduce/compact 摘要扁平索引 = codec canonical z-inner 布局（x + gx*(z + gz*y)）
			"tile_coord": SceneVoxelTileCodecScript.tile_coord_from_index(tile_index, tile_grid),
...
## GPU 三段归约（init→reduce→compact）出紧凑 tile 摘要；可复用契约传入的常驻 field buffer
func _reduce_scene_voxel_tile_summaries_gpu(
	...
	if use_prebound_collision_buffer:
		...
		collision_buffer = storage_buffer_from_bytes(
			SceneVoxelTileCodecScript.pack_collision_field_u32_bytes(collision_field, voxel_count),
			SCOPE_FRAME,
			"scene_voxel_tile_summary_collision_u32"
		)
...
func mark_scene_voxel_tile_dirty(tile_coord: Vector3i, dirty_flags = {}, source_record: Dictionary = {}) -> void:
	_touch_scene_voxel_tile(tile_coord, SceneVoxelTileCodecScript.flags_from_value(dirty_flags), source_record)
...
func apply_gpu_autoobject_dirty_delta(delta: Dictionary) -> Dictionary:
	...
	var object_id := VariantUtils.first_non_empty_string(delta, ["object_id", "auto_object_id", "auto_id", "id", "record_id"])
	...
	var has_current_min := SceneVoxelTileCodecScript.has_any_key(delta, SceneVoxelTileCodecScript.DELTA_CURRENT_MIN_KEYS)
	var has_current_max := SceneVoxelTileCodecScript.has_any_key(delta, SceneVoxelTileCodecScript.DELTA_CURRENT_MAX_KEYS)
	var has_previous_min := SceneVoxelTileCodecScript.has_any_key(delta, SceneVoxelTileCodecScript.DELTA_PREVIOUS_MIN_KEYS)
	var has_previous_max := SceneVoxelTileCodecScript.has_any_key(delta, SceneVoxelTileCodecScript.DELTA_PREVIOUS_MAX_KEYS)
...
func apply_gpu_autoobject_dirty_deltas(deltas: Array) -> Dictionary:
	...
	for raw_delta in deltas:
		...
		if raw_delta is Dictionary:
			...
			commit_result = apply_gpu_autoobject_dirty_delta(raw_delta as Dictionary)
