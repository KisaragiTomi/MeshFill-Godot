...
# MCP Interaction Server - TCP server for editor/game interaction
# Runs as an autoload, accepting JSON commands over TCP.
# @tool enables it in the editor so Godot MCP tools work without F6.
...
func _handle_command(json_str: String) -> void:
	...
	match command:
		...
		"eval_sync":
			_cmd_eval_sync(params)
		# Editor-only commands
		"editor_open_scene":
			_cmd_editor_open_scene(params)
		"editor_check_script":
			_cmd_editor_check_script(params)
		"editor_call_on_edited_root":
			_cmd_editor_call_on_edited_root(params)
		# Sync commands
		"mouse_move":
...
# --- Sync Eval (no await, works in @tool editor mode) ---
func _cmd_eval_sync(params: Dictionary) -> void:
	var code: String = params.get("code", "")
	if code.is_empty():
		_send_response({"error": "No code provided"})
		return
...
""" % [_indent_code(code)]
	...
	var result: Variant = obj.execute() if obj.has_method("execute") else null
	_send_response({"success": true, "result": _variant_to_json(result)})
...
# --- Change Scene ---
func _cmd_editor_open_scene(params: Dictionary) -> void:
	if not Engine.is_editor_hint():
		_send_response({"error": "editor_open_scene only works in editor mode"})
		return
	var scene_path: String = params.get("scene_path", "")
	if scene_path.is_empty():
		_send_response({"error": "scene_path is required"})
	...
	_send_response({"success": true, "opened": scene_path})


func _cmd_editor_check_script(params: Dictionary) -> void:
	...
	if script == null:
		_send_response({"error": "failed to load script", "script_path": script_path})
		return
	var methods := []
	...
	_send_response({
		...
		"is_valid": script.can_instantiate(),
...
func _cmd_editor_call_on_edited_root(params: Dictionary) -> void:
	if not Engine.is_editor_hint():
		_send_response({"error": "editor only"})
		return
	var method_name: String = params.get("method", "")
	...
	if root == null:
		_send_response({"error": "no edited scene root"})
		return
	if not root.has_method(method_name):
		var info := {
			"error": "method not found on edited root",
			"method": method_name,
			"root_name": root.name,
			"root_class": root.get_class(),
			"script_path": root.get_script().resource_path if root.get_script() else "none",
			"has_script_methods": root.get_script().get_script_method_list().size() > 0 if root.get_script() is GDScript else false,
		}
		_send_response(info)
		return
	var args: Array = params.get("args", [])
	var result: Variant = root.callv(method_name, args)
	_send_response({"success": true, "result": _variant_to_json(result)})
