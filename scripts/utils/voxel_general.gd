...
# ============================================================
# AABB 几何辅助（原困于 demo 作用域的 DemoAssets，及多处生产代码内联重复）
# ============================================================

## 用 Transform3D 变换 AABB：变换其 8 个角点后取世界包围盒。原 DemoAssets.transformed_aabb。
...
## AABB 三个轴向尺寸中的最大值（最长边）。原 DemoAssets.aabb_longest_axis 及多处内联 maxf。
...
## 规范化碰撞采样数组：跳过非字典/非点采样条目，深拷贝后补齐 voxel / collision_strength / weight 字段。
...
static func normalize_collision_samples(source: Array, _default_radius: float = 0.0) -> Array[Dictionary]:
	...
	for raw_collision in source:
		...
		if not is_point_collision_sample(collision):
			push_warning("normalize_collision_samples: ignoring non-point collision entry; box/cylinder primitives are no longer supported")
			continue
