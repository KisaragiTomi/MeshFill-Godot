...
// Expands per-asset tile votes into candidate route records/ranges,
// skipping tiles whose SceneVoxelTile summary has no content
// (max(scene_count, collision_count) == 0).
//
// This is the GPU-First replacement for the CPU readback chain:
//   CPU readback tile_summaries → CPU filter → GPU pack
// Because tile_summaries[] already resides in a PERSISTENT storage buffer,
// this shader reads it directly with zero CPU round-trip.
//
// Inputs:
...
//   candidate_route_counts[]    — u32[4]: written_records, positive_votes,
//                                  duplicate_marks, overflow_records
//   candidate_route_debug[]     — magic + diagnostics
...
layout(set = 0, binding = 4, std430) restrict buffer CandidateRouteRecords {
    uvec4 candidate_route_records[];
};

layout(set = 0, binding = 5, std430) restrict buffer CandidateRouteRanges {
...
void main() {
    ...
    uint skipped_empty_tiles = 0u; // diagnostic: tiles skipped because no scene/collision content

    if (asset_count == 0u || tile_count == 0u || record_capacity == 0u || summary_stride_uints < 8u) {
    ...
    for (uint asset_id = 0u; asset_id < asset_count; asset_id++) {
        ...
        // Clear tile marks for this asset
        ...
        for (uint center_id = 0u; center_id < tile_count; center_id++) {
            // --- Skip empty tiles ---
            // Use max(scene_count, collision_count) to determine if tile has content.
            // If both counts are zero, expansion is pointless — the score shader would discard it.
            if (!tile_has_content(center_id)) {
                skipped_empty_tiles++;
                continue;
            }
        ...
        // Write records and range for this asset
    ...
    candidate_route_debug[4] = 0x45535652u; // "ESVR" = Expand Scene Voxel Tile Routes
    ...
    candidate_route_debug[9] = skipped_empty_tiles;  // diagnostic counter
}
