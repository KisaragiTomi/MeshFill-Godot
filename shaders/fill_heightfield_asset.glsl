...
void main() {
    ...
    ivec2 current_size = imageSize(rw_current_scene_depth_a);
    bool in_texture = pos.x < current_size.x && pos.y < current_size.y;
    float max_cell = float(current_size.x);
    ...
    vec4 root_scene_color = vec4(0.0);
    vec4 target_height_color = vec4(0.0);
    if (in_texture) {
        root_scene_color = imageLoad(rw_current_scene_depth_a, pos);
        target_height_color = imageLoad(rw_target_height, pos);
    }
    ...
    bool is_active = in_texture && !(mask_check && (local_id.x != 0u || local_id.y != 0u)) && in_dirty;
