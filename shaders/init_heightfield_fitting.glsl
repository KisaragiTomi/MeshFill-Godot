...
layout(set = 0, binding = 4) uniform sampler2D t_placed_mask;
layout(set = 0, binding = 5) uniform sampler2D t_placement_override_mask;
layout(set = 0, binding = 6) uniform sampler2D t_placement_override_delta;
...
void main() {
    ...
    float previous_placement = texelFetch(t_placed_mask, pos, 0).r;
    float un_generate_mask = float(texelFetch(t_object_normal, pos, 0).z > 0.01 && obj_height > height);
    un_generate_mask = float(un_generate_mask > 0.0 || is_bound || height < 0.0 || previous_placement > 0.01);
    ...
    float ovr_mask = texelFetch(t_placement_override_mask, pos, 0).r;
    if (ovr_mask > 0.5) {
        float delta = texelFetch(t_placement_override_delta, pos, 0).r;
