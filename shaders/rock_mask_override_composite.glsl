...
layout(set = 0, binding = 0) uniform sampler2D base_mask_tex;
layout(set = 0, binding = 1) uniform sampler2D override_mask_tex;
layout(set = 0, binding = 2) uniform sampler2D override_delta_tex;

layout(r32f, set = 1, binding = 0) restrict writeonly uniform image2D out_mask_img;
...
void main() {
	...
	float value = texelFetch(base_mask_tex, p, 0).r;
	...
	if (mask_val >= params.threshold) {
		value = clamp(value + texelFetch(override_delta_tex, mp, 0).r * mask_val, 0.0, 1.0);
	}
	imageStore(out_mask_img, p, vec4(value, 0.0, 0.0, 0.0));
}
