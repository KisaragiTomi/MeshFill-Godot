...
`SceneVoxelTile` 管理 AutoObject 在 SV 两级（voxel + tile） 生命周期中的参与关系：object id 属于哪些 voxel 和 tile、对象变化 dirty 哪些 voxel/tile、以及 tile compact object range 如何发布。它不能回答“对象的完整运行时状态是什么”；完整 runtime object state 由 `GPUAutoObjectRuntime` 的 GPU object buffers 承担，资产默认语义由 [`AssetDescriptor`](../asset-descriptor-demo/asset-descriptor.md) 承担。
...
| object 信息 | 不进入 committed `SceneVoxel` accepted fields；object-ref index 保存 per-voxel object refs | 保存 tile 级 object id range（从 per-voxel refs 聚合） |
...
Heightfield rock fitting 是独立 producer，输出 placement results 后由 `main.gd` 实例化并派生 `ISWS`；详见 [`meshfill-rock-placement-flow.md`](../placement-meshfill-rock-placement/meshfill-rock-placement-flow2.5d(暂停开发).md)。Target-driven voxel region route 见 [`voxel-semantic-routing.md`](../placement-voxel-semantic-routing/voxel-semantic-routing.md)。
