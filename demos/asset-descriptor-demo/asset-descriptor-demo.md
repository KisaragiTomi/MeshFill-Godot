...
| 文档 | 关注点 |
| --- | --- |
| `res://demos/asset-descriptor-demo/asset-properties.md` | 共享字段 `color` / `complexity` / `collision` 的契约 |
| `res://demos/asset-descriptor-demo/auto-asset-scripting.md` | `AutoObject` 同名字段作为 Inspector / 兼容入口 |
| `res://demos/asset-descriptor-demo/asset-semantic-probes.md` | `SemanticProbeProfile` 探针分层采样与优先级 |
...
这些常量代表 `AssetDescriptor` 提供的默认语义，是探针的唯一输入来源。
...
`color` / `complexity` 是 canonical 共享字段；`channel` 等路由字段不属于共享语义。

## 探针调试

探针由 `SemanticProbeProfile.generate_from_mesh(mesh, collision, color, complexity, density, max_markers)` 生成，按 `shape_source` 分优先级分层 top-k 采样。本 demo 把 GPU 体素化结果（`MeshVoxelizerGpu`）中 `collision > 0` 的体素作为逐体素采样点传入 `collision`，每个点携带同级的 `color` / `complexity` / `collision`：

| shape_source | 优先级 | 来源 | marker 颜色 |
| --- | --- | --- | --- |
| `convex` | 3（最高） | 凸包表面点 | 黄 `Color(1.0, 0.85, 0.1)` |
| `voxel_interior` | 2 | 通用 voxel 场实体体素（逐体素 color/complexity/collision） | 蓝 `Color(0.15, 0.75, 1.0)` |
...
| `C` | 清除所有调试节点（探针 + buffer info） |
| `B` | 切换 Buffer Info 叠加层 |

## 验证步骤

1. 在 Godot 打开场景，确认 4 树 + 4 岩石及参考地面正常渲染。
2. 按 `1` / `2` / `3`，确认探针球贴合资产几何，且 `convex`（黄）/ `surface`（绿）颜色区分正确。
3. 按 `B`，确认叠加层显示资产数量与 `color` / `complexity` / `density` 参数。
4. 按 `C`，确认所有调试节点被清除、再次按键可重新生成。
