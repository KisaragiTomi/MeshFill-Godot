...
![Voxel semantic routing overview((voxel-semantic-routing.svg)
...
- [ ( 继续减少 `candidate_voxel_regions_by_asset` CPU readback 对正常 placement path 的依赖，只保留 debug / entry；legacy `candidate_voxel_sparses_by_asset` 不作为新接口扩展。
...
设计文档：[`target-scene-voxel-projection.md`((target-scene-voxel-projection.md)
...
设计文档：[`asset-semantic-probes.md`((../core/asset-semantic-probes.md)
