...
func get_contract_source_docs() -> Array:
	var source_docs := []
	if has_meta("source_doc"):
		_append_unique(source_docs, str(get_meta("source_doc")).strip_edges())
	if has_meta("source_docs"):
		for part in str(get_meta("source_docs")).split(";"):
			_append_unique(source_docs, String(part).strip_edges())
	return source_docs
