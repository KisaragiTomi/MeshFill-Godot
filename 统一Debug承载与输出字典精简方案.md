...
>
> **剩余工作（按价值排序）**：~~① VPG debug_voxel 收编 ChannelField 实例~~（✅ `e496ac4`）② writeback 报告走 `ReportSchema.build`（expand/migrate/contract 三步）③ VPG run 输出定义 schema + diag 分级 ④ TARGET_STATS 迁移（先加 decode kind）⑤ golden-master 消费链（tools/ 桥客户端 + `goldens/` 存储对比）⑥ 观测实例写侧门控（顺带解决 Open Question 1 的 bitmask 粒度）。
